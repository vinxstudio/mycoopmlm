...
if (!empty($str_account_ids)) {
	...
	$sql_query = "SELECT earnings.user_id, user_details.first_name FROM earnings LEFT JOIN users ON users.id = earnings.user_id LEFT JOIN user_details ON user_details.id = users.user_details_id GROUP BY earnings.user_id ORDER BY user_details.first_name ASC";
	$sql = $conn->query($sql_query);
	$earnings = $sql->fetch_all();
	...
	foreach ($earnings as $earning) {
		...
		#get group id
		$group_id = getGroupId($user_id, $conn);
		...
		$total_DR = calculate_income($user_id, 'direct_referral', $date_from, $date_to, $conn);
		$total_MB = calculate_income($user_id, 'pairing', $date_from, $date_to, $conn);
		$total_GC = calculate_income($user_id, 'GC', $date_from, $date_to, $conn);
		...
		if ($total_income > 0) {
			...
			overallTotalIncome($group_id, $user_id, $total_income, $direct_referral, $matching_bonus, $gift_check, $date_from, $date_to, $conn);
		}
...
function getGroupId($user_id, $conn)
{
	$getGroupIdSql = "SELECT group_id FROM users WHERE id = {$user_id}";
	$sql = $conn->query($getGroupIdSql);
...
function calculate_income($user_id, $source, $date_from, $date_to, $conn)
{
	$sql = $conn->query("SELECT SUM(amount) as amount, user_id FROM earnings WHERE user_id = {$user_id} AND source = '{$source}' AND created_at >= '{$date_from}' AND created_at <= '{$date_to}'");
...
function overallTotalIncome($group_id, $user_id, $total_income, $direct_referral, $matching_bonus, $gift_check, $from, $to, $conn)
{	
	#deduct 10%
	$deduction = $total_income * 0.1;
	$new_income = $total_income - $deduction;
	#check if already inserted
	$is_inserted = chck_weekly_payout($group_id, $user_id, $from, $to, $conn);
...
function chck_weekly_payout($group_id, $user_id, $from, $to, $conn)
{
	$sql = "SELECT * FROM weekly_payout 
					WHERE group_id = {$group_id} AND user_id = {$user_id} AND date_from = '{$from}' AND date_to = '{$to}'";
...
function insert_new($conn, $table, $insData = array())
{
	if (!empty($insData)) {
		...
		if ($conn->query($query) === true) {
			echo "New Payout record created successfully" . PHP_EOL;
		} else {
