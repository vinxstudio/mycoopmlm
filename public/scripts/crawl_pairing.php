...
if (!empty($str_account_ids)) {
	...
	foreach ($account_ids as $account_id_row) {
		...
		#process points summary
		// process_points_summary($account_id, $left_downlines['total_points'], $right_downlines['total_points'], $conn);

		#process points value
	...
	if ($getDownlinesresult->num_rows > 0) {
		...
		while ($dowlinecountrow = $getDownlinesresult->fetch_assoc()) {
			...
			if ($thisUserrow[16] > 3) {
				...
				$points_retention = points_retention($this_account['id'], $dowlinecountrow['account_id'], $node, $conn);
				if (!$points_retention) {
...
function process_points_value($this_account, $LPV_amount, $RPV_amount, $strong_leg, $conn)
{
	...
	if ($is_exist) {
		...
		insert_new($conn, 'points_value', $data);
	}
}

function chk_points_checker($this_accountId, $conn)
{
	$sql = "SELECT * FROM points_value WHERE account_id = {$this_accountId}";
...
{
	$sql = $conn->query("SELECT * FROM points_value WHERE account_id={$this_accountId}");
	if ($sql->num_rows > 0) {
		$PV = $sql->fetch_assoc();
	} else {
		$PV = false;
			...
			if ($upline_info[1] != 300) {
				...
				if ($cntr >= 5 && $cntr % 5 == 0) {
					...
					$data_pp = array(
						...
						'strong_leg' => $strong_leg,
						'mb_amount' => $mb_amount / 2,
						'created_at' => date('Y-m-d H:i:s')
					);
					insert_new($conn, 'points_retention', $data_pp);
...
{
	$sql = "SELECT account_id, earned_date FROM earnings_pairing 
...
function get_earnings($account_id, $earned_date, $conn)
{
	...
	$sql = "SELECT * FROM earnings_pairing 
					WHERE account_id = {$account_id} AND source IN('pairing', 'GC') 
			...
			if ($i > 7) {
				...
				if (!$is_already_flushout) {
					$data = array(
						...
						'left_account_id' => $earning->left_account_id,
						'right_account_id' => $earning->right_account_id,
						'earned_date' => $earning->earned_date,
						'created_at' => $earning->created_at
					);

					insert_new($conn, 'earnings_flushout', $data);

					#update earnings flushout source
					$data = "source = 'flushout'";
					$where = " id = {$earning->id} AND earned_date >= '2018-07-18' ";
					update_table($conn, 'earnings_pairing', $where, $data);
				}
			}
...
{
	$sql = "SELECT * FROM earnings_flushout 
					WHERE id = {$id}";
